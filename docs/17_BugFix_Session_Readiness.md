...
3. **배치3 "Flyway V25"는 무효.** V25는 블록 B가 선점했다. 같은 번호로 파일을 만들면 로컬 dev DB가 checksum mismatch로 부팅 실패하고 프로드는 조용히 통과 — 최악의 분기. **신규는 V26부터**, 관심사별 분리. ⚠ **갱신(2026-08-20): V26은 블록 D가 선점했다** (`V26__drop_bpm_not_null.sql` — §G-8 BPM 폐지 선행 스키마). 버그픽스 세션은 **V27부터**: V27 결제·구독 / V28 에너지 분할 / V29 일러·극장.
...
| **1B DB 무결성** | **V27**(orders.imp_uid unique + 구독 부분 유니크) · D-4 구독 정합 3건 | BE+DB | 배치 1 직후 단독 리비전 |
| **2 결제·인증 개통** | C-1.3~1.5 · C-2 전량 · PortOne 초기화 이관 | BE+FE+INFRA | **행정 선행 확인 필요** |
| **3 자산 손실 정지** | V27 EnergySplit · D-1 호출부 7곳 · D-2 · D-3 UGC 좀비잡 · D-6 | BE 단독 | ✔ |
