...
public enum ErrorCode {
    ...
    /**
     * [E-4.4] 이미 마감된 챕터에 {@code POST /chapter-end}가 다시 들어옴.
     *
     * <p>서버 입장에선 거부지만 <b>클라이언트 입장에선 이미 원하는 상태</b>다(챕터가 끝났다).
     * 그래서 FE는 이것을 에러 토스트로 띄우지 말고 <b>자기 치유</b>해야 한다 —
     * 응답이 유실된 재시도가 정확히 이 코드로 돌아오는데, 거기서 유저를 세우면
     * '마무리하는 중…' 오버레이에 영구히 갇힌다(E-1.1이 바로 그 증상이었다).
     *
     * <p>⚠ {@code UNPAID_BATCH}와 같은 계열의 규율 — 새 거부 코드를 만들 때는
     * FE 복구 경로를 <b>같은 커밋에서</b> 배선한다.
     */
    CHAPTER_ALREADY_FINALIZED,
